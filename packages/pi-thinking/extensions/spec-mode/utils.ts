/**
 * Pure utility functions for spec mode / plan mode compatibility.
 * Extracted for testability.
 */

// Destructive commands blocked in spec mode
...
// Safe read-only commands allowed in spec mode
const SAFE_PATTERNS = [
	/^\s*cat\b/,
	/^\s*head\b/,
	/^\s*tail\b/,
...
export function extractTodoItems(message: string): TodoItem[] {
	...
	// Flexible header detection: supports "Spec:", "**Spec:**", "## Implementation Spec", etc.
	const headerMatch = message.match(/(?:^|\n)\s*(?:#{1,6}\s+)?\*{0,2}(?:[\w]+\s+)*Spec[\s:*]*\n/im);
